...
void ImageNodelet::onInit()
{
  ...
  pub_ = nh.advertise<image_view_plus::Click>(nh.resolveName("click"), 10);
}
...
void ImageNodelet::mouseCb(int event, int x, int y, int flags, void* param)
{
  ...
  {
    image_view_plus::Click click;
    click.x = x;
    click.y = y;
    click.flags = flags;
    this_->pub_.publish(click);
    return;
  }
...
PLUGINLIB_DECLARE_CLASS(image_view_plus, image, image_view::ImageNodelet, nodelet::Nodelet)
